...
static TIMER_watch watches[TIMER_WATCHTYPE_LAST];

TIMER_watch_formated TIMER_active_watch_formated = {.c_str=" 0:00:00:00"};//h:mm:ss:cs
enum TIMER_WATCHTYPE TIMER_active_watchtype;
...
TESTUSE static uint8_t TESTADDPREFIX(increment)(TIMER_watch* watch, TIMER_centisecond_t centiseconds)
{
    ...
    uint8_t format_flag = TIMER_FORMATFLAG_NONE;
    while(centiseconds || clipped_centiseconds)
    {
        format_flag |= TIMER_FORMATFLAG_CENTISECONDS;
        ...
        if(FULL_SECOND == watch->timer.centiseconds)
        {
            ...
            watch->timer.seconds++;
            format_flag |= TIMER_FORMATFLAG_SECONDS;  
            if(watch->timer.seconds == 60)
            {                
                watch->timer.minutes++;
                format_flag |= TIMER_FORMATFLAG_MINUTES;
                watch->timer.seconds = 0;
                if(watch->timer.minutes == 60)
                {
                    watch->timer.hours++;
                    format_flag |= TIMER_FORMATFLAG_HOURS;
...
static void switch_watchtype(enum TIMER_WATCHTYPE watchtype)
{
    active_watch = &watches[watchtype];
    TIMER_active_watchtype = watchtype;
}

/*Format input timer to string based formated string*/
void TIMER_format(TIMER_watch* timer, TIMER_watch_formated* formated, enum TIMER_FORMATFLAG format_flag)
{
    switch(format_flag)
    {
        case TIMER_FORMATFLAG_HOURS:
            *formated->segments.hh = ' ';
            rightconcat_short(formated->segments.hh,timer->timer.hours,2);
        /*FALLTHROUGH*/
        case TIMER_FORMATFLAG_MINUTES:
            *formated->segments.mm = '0';
            rightconcat_short(formated->segments.mm,timer->timer.minutes,2);
        /*FALLTHROUGH*/
        case TIMER_FORMATFLAG_SECONDS:
            *formated->segments.ss = '0';
            rightconcat_short(formated->segments.ss,timer->timer.seconds,2);
        /*FALLTHROUGH*/
        case TIMER_FORMATFLAG_CENTISECONDS:
...
{
    if(active_watch->timer.watchstatus == TIMER_WATCHSTATUS_COUNTING)
    {
        active_watch->timer.watchstatus = TIMER_WATCHSTATUS_STOP;
    }
    else
        active_watch->timer.watchstatus = TIMER_WATCHSTATUS_COUNTING;
}

void TIMER_clear_active_watch()
{
    memset(&active_watch->timer,0x0,sizeof(active_watch->timer));
    active_watch->timer.watchstatus = TIMER_WATCHSTATUS_ZERO;
    TIMER_format(active_watch, &TIMER_active_watch_formated, TIMER_FORMATFLAG_HOURS);
}

void TIMER_next_watch()
{   
    uint8_t watchtype = TIMER_active_watchtype + 1;
    if(watchtype == TIMER_WATCHTYPE_LAST)
        watchtype = 0;
    TIMER_set_watch(watchtype);
}

uint8_t TIMER_set_watch(enum TIMER_WATCHTYPE watchtype)
{
    if(watchtype < TIMER_WATCHTYPE_LAST)
    {
        switch_watchtype(watchtype);
        TIMER_format(active_watch, &TIMER_active_watch_formated, TIMER_FORMATFLAG_HOURS);
        return 1;
    }
    return 0;
...
TIMER_watch* TIMER_get_watch(enum TIMER_WATCHTYPE watchtype)
{ 
    TIMER_watch* watchptr = 0x0;
    if(watchtype < TIMER_WATCHTYPE_LAST)
    {
        watchptr = &watches[watchtype];
    }
    return watchptr;
}
...
void TIMER_userinput_handle_watch(INPUT_Event* input_event)
{
    ...
    {
        if(TIMER_WATCHTYPE_STOPWATCH == TIMER_active_watchtype)
        {
            ...
            else if(INPUT_KEYSTATUS_CLICK == input_event->keystatus)
            {
                ...
                else if(TIMER_WATCHSTATUS_COUNTING == active_watch->timer.watchstatus)
                {
        ...
        else if(TIMER_WATCHTYPE_WATCH == TIMER_active_watchtype)
        {
            ...
            if(0 < watchsetup_counter)
            {
                ...
                */
                ...
                TIMER_watch* watch = &watches[TIMER_WATCHTYPE_WATCH];
                if(TIMER_WATCHSTATUS_STOP == watch->timer.watchstatus && INPUT_KEYSTATUS_RELEASED == input_event->keystatus)
                {
                    /*
                    ...
                    */
                    watches[TIMER_WATCHTYPE_WATCH].timer.watchstatus = TIMER_WATCHSTATUS_SETUP;
                }
                if(TIMER_WATCHSTATUS_SETUP == watch->timer.watchstatus)
                {
            ...
            else
            {
                ...
                if(INPUT_KEYSTATUS_HOLD == input_event->keystatus)
                {
                    ...
                    stop_watch(&watches[TIMER_WATCHTYPE_WATCH]);
                    watchsetup_counter = watchsetup_duration;
                }
            }
        }
        ...
        if(0 == watchsetup_counter)
        {
            ...
            */
            for(uint8_t i = 0;i < WATCHSETUPDURATION_SECONDS; i++)
                increment(&watches[TIMER_WATCHTYPE_WATCH],FULL_SECOND);

            start_watch(&watches[TIMER_WATCHTYPE_WATCH]);
        }
    }
    TIMER_watch* watch;
    uint8_t format_flag = 0;
    for(uint8_t i = 0;i < TIMER_WATCHTYPE_LAST;i++)
    {
        watch = &watches[i];
...
{
    switch_watchtype(TIMER_WATCHTYPE_WATCH);
    watches[TIMER_WATCHTYPE_WATCH].timer.watchstatus = TIMER_WATCHSTATUS_COUNTING;
}